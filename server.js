...
let feedback = [];  // [{ id, guestId, name, rating, comment, marketingOk, ts }]
let settings = { locFreqSec: 8 };  // admin-configurable; delivered to drivers
function save(){ try{ fs.writeFileSync(DB_FILE, JSON.stringify({ guests, drivers, events, feedback, settings })); }catch(e){} }
try{ if(fs.existsSync(DB_FILE)){ const d=JSON.parse(fs.readFileSync(DB_FILE,"utf8"));
  ...
  if(Array.isArray(d.feedback)) feedback=d.feedback;
  if(d.settings && d.settings.locFreqSec) settings=d.settings;
} }catch(e){}
...
function snapshot(){ return { guests, drivers, events: events.slice(-400), feedback: feedback.slice(-200), settings, venue: VENUE }; }
...
io.on("connection", (socket) => {
  ...
  // ---- admin sets the location-update frequency (delivered to drivers) ----
  socket.on("saveSettings", (rec) => {
    if (rec && rec.locFreqSec){ settings.locFreqSec = Math.max(3, Math.min(60, +rec.locFreqSec)); save(); io.emit("settings", settings); }
  });
  ...
  // ---------- DRIVER ----------
  socket.on("driverHello", ({ id }) => { if (drivers[id]){ socket.data.driverId=id; socket.join("driver:"+id); socket.emit("youAre", drivers[id]); } });

  // ---- admin or driver creates / edits a driver (name, vehicle, photo, code) ----
  socket.on("saveDriver", (rec, ack) => {
    if (!rec){ if (typeof ack==="function") ack({ ok:false }); return; }
    let d;
    if (rec.id && drivers[rec.id]) {
      d = drivers[rec.id];
      if (rec.name) d.name = String(rec.name).slice(0,40);
      if (rec.phone != null) d.phone = String(rec.phone).slice(0,30);
      if (rec.code != null) d.code = String(rec.code).slice(0,12);
      if (rec.photo != null) d.photo = String(rec.photo).slice(0, 400000); // small data URL ok
      if (rec.vehicle) d.vehicle = { ...(d.vehicle||{}), ...rec.vehicle };
    } else {
      const id = rec.id || ("D" + Date.now());
      d = { id, name: rec.name || "New driver", phone: rec.phone||"", code: rec.code || String(1000+Math.floor(Math.random()*8999)),
            photo: rec.photo||"", status:"available", lat:VENUE.lat, lng:VENUE.lng, heading:0, ts:0,
            vehicle: rec.vehicle || { make:"", model:"", color:"", plate:"", capacity:4, luggage:2 } };
      d.id = id; drivers[id] = d;
    }
    save(); io.emit("driver", d); io.emit("snapshot", snapshot());
    if (typeof ack==="function") ack({ ok:true, driver:d });
  });
  socket.on("deleteDriver", (rec) => {
    const id = rec && rec.id; if (!id || !drivers[id]) return;
    // unassign any guests on this driver
    Object.values(guests).forEach(g=>{ if (g.driverId===id){ g.driverId=null; io.emit("guest", g); } });
    delete drivers[id]; save(); io.emit("driverGone", { id }); io.emit("snapshot", snapshot());
  });

  // ---------- GUEST (for live tracking page + calling) ----------
  socket.on("guestHello", ({ id }) => { if (id){ socket.data.guestId=id; socket.join("guest:"+id); } });

  // ---------- CLICK-TO-CALL: WebRTC signaling (peer-to-peer audio, no paid bridge) ----------
  // Messages carry { guestId, driverId, target:'driver'|'guest', ... }. We just relay them
  // to the right room; the actual audio flows directly phone-to-phone over WebRTC + public STUN.
  function relayCall(ev, m){
    const room = (m && m.target==="driver") ? ("driver:"+m.driverId) : ("guest:"+m.guestId);
    socket.to(room).emit(ev, m);
  }
  socket.on("callOffer",   (m)=> relayCall("callOffer", m));
  socket.on("callAnswer",  (m)=> relayCall("callAnswer", m));
  socket.on("callIce",     (m)=> relayCall("callIce", m));
  socket.on("callEnd",     (m)=> relayCall("callEnd", m));
  socket.on("callDecline", (m)=> relayCall("callDecline", m));
  socket.on("callRinging", (m)=> relayCall("callRinging", m));

  // ---------- FEEDBACK: "How'd we do?" (+ marketing consent) ----------
  socket.on("feedback", (m, ack)=>{
    if(!m || !m.rating){ if(typeof ack==="function") ack({ok:false}); return; }
    const g = guests[m.guestId] || {};
    const f = { id:"F"+Date.now()+Math.random().toString(36).slice(2,5), guestId:m.guestId||null,
      name:(g.firstName?g.firstName+" "+(g.lastName||""):"Guest"), rating:Math.max(1,Math.min(5,+m.rating)),
      comment:(m.comment||"").slice(0,500), marketingOk:!!m.marketingOk, ts:Date.now() };
    feedback.push(f); if(feedback.length>1000) feedback=feedback.slice(-1000);
    io.emit("feedback", f);
    logEvent(m.guestId, "feedback", "★".repeat(f.rating)+(f.marketingOk?" · may use for marketing":""));
    save(); if(typeof ack==="function") ack({ ok:true });
  });
